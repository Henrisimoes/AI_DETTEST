...
import locale
import os
import re # Importar re para expressões regulares (sanitização de filename)
...
def sanitize_filename(filename):
    ...
    # Um nome de arquivo muito longo pode causar FileNotFoundError mesmo com caracteres válidos
    if len(filename) > 150: # Você pode ajustar este limite conforme necessidade
...
def gerar_opcoes_marcadas(opcao_escolhida, opcoes_dict):
    ...
    max_label_len = 0
    if opcoes_dict: # Garante que não está vazio
...
def gerar_dfd_completo(dados_formulario, lista_itens):
    # Caminho do template: assumindo que 'templates' está na mesma pasta do script ou na raiz.
    ...
    dados_para_template = {
        **dados_formulario,
        "lista_itens": lista_itens  # A lista de itens que será iterada no template docx
    }
    ...
    # Pega o item para o nome do arquivo, com fallback para "documento" se não houver
    item_para_nome = dados_formulario.get("item", "documento") 
    
    # Sanitiza o nome do item para garantir que seja um nome de arquivo válido
    nome_sanitizado = sanitize_filename(item_para_nome)

    # Gera o nome completo do arquivo com timestamp para evitar sobrescrever
    ...
    # Renderiza o documento com os dados e salva
